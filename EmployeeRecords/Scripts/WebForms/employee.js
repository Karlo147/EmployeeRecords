...
function get() {
    ...
    //$('#tbody').append("<tr class=\"loading\"><td colspan=\"8\"><img src=\"Content/img/load.gif\" /></td></tr>");
    ...
    (new http).post("employees.aspx/get", {
        ...
        var items = response.d.map(item => {
            return new Promise(function (resolve, reject) {
                ...
                var active = "<i data-id=\"" + item.EmployeeNumber + "\" data-name=\"" + item.FirstName + " " + item.LastName + "\" class=\"fa fa-remove remove\">" +
                                  // "<span class=\"tooltiptext\">Click to deactivate</span>" +
                               "</i>";
                var inactive = "<i data-id=\"" + item.EmployeeNumber + "\" data-name=\"" + item.FirstName + " " + item.LastName + "\" class=\"fa fa-check-circle activate\">" +
                ...
                var html = "<tr>" +
                           ...
                           "<td><img class='img-responsive' src=\"Content/img/ids/emp-placeholder.png\"/></td>" +
                           //"<td>Image Here!</td>" +
                           ...
                           "<td>" + isActive + "</td>" +
                           "<td>" +
                               "<i data-id=\"" + item.EmployeeNumber + "\" class=\"fa fa-search view\" data-toggle='modal' data-target='#viewModal'> " +
                                   "<span class=\"tooltiptext\">Click to view details</span>" +
                               "</i>";
                html += "<i data-id=\"" + item.EmployeeNumber + "\" class=\"fa fa-edit edit\" data-toggle='modal' data-target='#editModal'>" +
...
$(document).on('click', '#save', function (e) {
    ...
    if (isInputValid()) {
        ...
        (new http).post("instructors.aspx/insert", {
            fname: $('#fname').val(),
            lname: $('#lname').val(),
            mname: $('#mname').val(),
            email: $('#email').val(),
